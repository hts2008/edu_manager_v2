  ...
  const columns = [
    {
      ...
      render: (value, row) => (
        <div>
          ...
          <p className="text-xs text-gray-500">{row.level || 'Chưa xác định'}</p>
        </div>
      ),
    },
    { key: 'teacher_name', title: 'Giáo viên' },
    {
      ...
      render: (value) => {
        try {
          const days = typeof value === 'string' ? JSON.parse(value || '[]') : (value || []);
          const dayNames = { 2: 'T2', 3: 'T3', 4: 'T4', 5: 'T5', 6: 'T6', 7: 'T7', 1: 'CN' };
    ...
    },
    {
      ...
      title: 'Học viên',
      render: (value) => (
        <span className="badge-info">{value || 0}</span>
      ),
    },
    {
      key: 'status',
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          <button
            onClick={(e) => { e.stopPropagation(); setEditingClass(row); setShowForm(true); }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setSelectedClass(row); setShowDeleteConfirm(true); }}
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        data={classes}
        loading={loading}
        onRowClick={(row) => { setEditingClass(row); setShowForm(true); }}
...
function ClassForm({ classData, teachers, onSuccess, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Học phí/buổi (VND)</label>
          <input
            type="number"
            value={formData.fee_per_day}
            onChange={(e) => setFormData({ ...formData, fee_per_day: parseInt(e.target.value) || 0 })}
            className="input"
          />
        </div>
        <div>
