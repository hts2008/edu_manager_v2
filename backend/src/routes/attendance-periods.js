...
router.post("/", (req, res, next) => {
  try {
    ...
    console.log("[DEBUG] Create period request:", {
      class_id,
      month,
      types: { class_id: typeof class_id, month: typeof month },
    });

    if (!class_id || !month)
      throw new AppError("class_id and month required", 400);

    // Ensure both are strings
    const classIdStr = String(class_id);
    ...
    if (!period) {
      const id = `AP${monthStr.replace("-", "")}${classIdStr.slice(-4)}`;
      console.log("[DEBUG] Creating period with id:", id);
      execute(
        `
        INSERT INTO attendance_periods (id, class_id, period_month, status)
        VALUES (?, ?, ?, 'open')
      `,
        [id, classIdStr, monthStr]
      );
    ...
    console.error("[DEBUG] Period creation error:", error.message);
    next(error);
  }
});
...
router.post("/:id/submit", (req, res, next) => {
  try {
    const period = queryOne("SELECT * FROM attendance_periods WHERE id = ?", [
      req.params.id,
    ]);
    if (!period) throw new AppError("Period not found", 404);
...
router.post("/:id/approve", adminOnly, (req, res, next) => {
  try {
    ...
    if (period.status !== "submitted")
      throw new AppError(
        `Cannot approve: current status is ${period.status}`,
        400
      );
...
router.post("/:id/lock", adminOnly, (req, res, next) => {
  try {
    ...
    students.forEach((s) => {
      ...
      if (!existing) {
        const feeId = `MF${period.period_month.replace(
          "-",
          ""
