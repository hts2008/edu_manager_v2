...
// GET /api/classes OR GET /api/classes?id=xxx
router.get("/", (req, res, next) => {
  try {
    const { id } = req.query;

    // If id is provided, return single class with students (for getById)
    if (id) {
      const cls = queryOne(
        `
      ...
      return res.json({
        success: true,
        data: { ...cls, schedule_days: scheduleDays, students },
      });
    }
...
// PUT /api/classes?id=xxx (for frontend compatibility)
router.put("/", adminOnly, (req, res, next) => {
  try {
    const { id } = req.query;
    if (!id) throw new AppError("Class ID is required", 400, "BAD_REQUEST");
    ...
    if (!queryOne("SELECT id FROM classes WHERE id = ?", [id]))
      throw new AppError("Class not found", 404, "NOT_FOUND");

    const scheduleDaysJson = schedule_days
      ? JSON.stringify(schedule_days)
      : null;
    execute(
      `UPDATE classes SET class_name = COALESCE(?, class_name), schedule_days = COALESCE(?, schedule_days),
    ...
    res.json({
      success: true,
      data: queryOne("SELECT * FROM classes WHERE id = ?", [id]),
    });
  } catch (error) {
    next(error);
  }
...
// DELETE /api/classes?id=xxx (for frontend compatibility)
router.delete("/", adminOnly, (req, res, next) => {
  try {
    const { id } = req.query;
    if (!id) throw new AppError("Class ID is required", 400, "BAD_REQUEST");
    ...
    const enrolledCount = queryOne(
      "SELECT COUNT(*) as count FROM student_classes WHERE class_id = ? AND status = 'active'",
      [id],
    );
    ...
    res.json({ success: true, message: "Class deleted", data: { id } });
  } catch (error) {
    next(error);
  }
});
...
router.post("/", adminOnly, (req, res, next) => {
  try {
    ...
    const count = queryOne(
      "SELECT COUNT(*) as c FROM classes WHERE id LIKE ?",
      [`CLS${today}%`],
    );
    const id = `CLS${today}${String(count.c + 1).padStart(3, "0")}`;

    execute(
      `INSERT INTO classes (id, class_name, schedule_days, start_time, end_time, fee_per_day, max_students, teacher_id, notes) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        class_name,
...
router.delete("/:id", adminOnly, (req, res, next) => {
  try {
    if (!queryOne("SELECT id FROM classes WHERE id = ?", [req.params.id]))
      throw new AppError("Class not found", 404, "NOT_FOUND");
    execute("UPDATE classes SET status = 'inactive' WHERE id = ?", [
      req.params.id,
    ]);
...
router.post("/:id/enroll", (req, res, next) => {
  try {
    ...
    if (!queryOne("SELECT id FROM classes WHERE id = ?", [class_id]))
      throw new AppError("Class not found", 404);
    if (!queryOne("SELECT id FROM students WHERE id = ?", [student_id]))
      throw new AppError("Student not found", 404);

    execute(
      `INSERT INTO student_classes (student_id, class_id, enrollment_date) VALUES (?, ?, date('now'))`,
      [student_id, class_id],
    );
