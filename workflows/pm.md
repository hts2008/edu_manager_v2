...
FOR each task in dependency order:
  ...
  Execute:
    IF task requires deep focus → spawn CLI subagent
    ELSE → execute inline
...
2.5. Post-Task Consolidation (§21.3 step 6-7):
     IF (now - last_consolidation) < 2 min:
       SKIP → "Consolidation skipped — ran recently"
     ELSE IF task_risk >= MEDIUM OR completed_count_since_consolidation >= 2:
       nmem_consolidate(strategy="enrich", compact=true)  → Tier 1 (<1s)
     ELSE IF completed_count_since_consolidation >= 3 OR grade < B:
       nmem_consolidate(strategy="all", compact=true)     → Tier 2 (5-30s)
     nmem_health(compact=true) → log grade in delivery report
     TIMEOUT: 30s → abort, continue to step 3
     ON FAIL: log warning, continue delivery (NEVER block)
...
| CLI subagent quota exhausted | Fall back to native execution |
