...
0. **Attendance week selection during metadata refresh (IMPLEMENTED + PROD 2026-07-17)** - Fixed the residual operator-reported issue where clicking past/future month week rows did not open the weekly attendance panel when `weekLoading` was true. Calendar selection now uses `weekSelectionDisabled = loading || saving`, while save/cell mutation remains gated by `attendanceControlsDisabled = loading || weekLoading || saving`. Commit `2867171` deployed as Vercel `dpl_C9Kya8T288BGNLpRBd7V3UF8Udgn`. Authenticated production Chrome selected `FLYER B6`, clicked `attendance-week-2026-06-1`, rendered `Điểm danh tuần: 1/6/2026 - 7/6/2026`, and after metadata load the save button was enabled with no API 500s. Evidence: `receipts/2026-07-17-attendance-week-selection-metadata-refresh.md`.
